 ...
 */
public class MeteringConstantUnsupportedOperationParser extends AbstractXContentParser {
    private final XContentParser delegate;
    private final AtomicLong counter;

    static final int ASCII_MAX = 0x7F; // 127 1byte
    static final int BMF_MAX = 0xFFFF; // end of BMP Character.MAX_VALUE - this is probably legacy
    ...
    private void charge(Token token) throws IOException {
        if (token != null) {
            var sizeInBits = switch (token) {
                ...
                case VALUE_EMBEDDED_OBJECT -> calculateBase64Length(binaryValue().length) * 8;
    ...
    private int calculateTextLength() throws IOException {
        ...
        for (int i = textOffset(); i < limit;) {
            ...
            } else if (codePoint <= BMF_MAX) {
                byteLength += 2; // Two bytes for characters in the range U+0080 to 0xFFFF
            } else {
    ...
    public MeteringConstantUnsupportedOperationParser(XContentParser xContentParser, AtomicLong counter) {
